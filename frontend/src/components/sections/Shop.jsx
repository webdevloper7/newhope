...
const Shop = () => {
  ...
  return (
    <section id="shop" className="py-24 bg-gray-900 text-white overflow-hidden">
      <div className="container mx-auto px-6 lg:px-12">
        <div className="flex flex-col lg:flex-row justify-between items-end mb-16 gap-8">
          ...
          <a 
            href={`https://wa.me/91${phone}?text=${encodeURIComponent("Hello! I'd like to see more products available at your clinic.")}`}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 bg-white text-gray-900 px-8 py-4 rounded-2xl font-black hover:bg-primary hover:text-white transition-all group"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {products.map((p) => (
            <div key={p.id} className="group bg-gray-800/50 rounded-[2rem] p-4 border border-white/5 hover:border-primary/30 transition-all duration-500">
              <div className="relative aspect-square rounded-2xl overflow-hidden mb-6">
                ...
                />
                <div className="absolute top-4 left-4 bg-primary text-white text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-full shadow-lg flex items-center gap-1">
                  <Tag size={10}/> {p.tag}
                </div>
              </div>
              <div className="px-2 space-y-3">
                ...
                <h4 className="text-xl font-bold tracking-tight group-hover:text-primary transition-colors">{p.name}</h4>
                <div className="flex items-center justify-between pt-2">
                  <span className="text-2xl font-black text-primary">{p.price}</span>
                  <button 
                    onClick={() => handleInquiry(p)}
                    className="p-3 bg-white/5 rounded-xl hover:bg-success transition-colors flex items-center gap-2 group/btn"
                    title="Inquiry on WhatsApp"
                  >
                    <MessageCircle size={20} className="text-success group-hover/btn:text-white" />
                  </button>
                </div>
              </div>
            </div>
